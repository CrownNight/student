...
export default class NewsListLeft extends React.Component{
    ...
    getNewsList(index,size){
        webApi.get('/getNewsList?index='+index+'&size='+size+'&type=new').then(data=>{
            if(data.flag){
    ...
    render(){
        ...
        return(
            <div>
                ...
                renderItem={item => (<List.Item><Link to={{pathname:'/front/news/info',state:item}}>{item.newsTitle}</Link></List.Item>)}
                />
            </div>
        )
    }
